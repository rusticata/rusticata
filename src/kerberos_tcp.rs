...
impl RBuilder for KerberosTCPBuilder {
    fn build(&self) -> Box<dyn RParser> {
        Box::new(KerberosParserTCP::new(b"Kerberos/TCP"))
    }
    fn get_l4_probe(&self) -> Option<ProbeL4> {
        Some(kerberos_probe_tcp)
    }
}
    ...
    fn parse_l4(&mut self, data: &[u8], direction: Direction) -> ParseResult {
        ...
        let tcp_buffer = match self.record_ts {
            ...
            _ => {
                ...
                if self.defrag_buf_ts.len() + data.len() > 100_000 {
                    warn!(
                        "krb5_parse_tcp: TCP buffer exploded {} {}",
                        self.defrag_buf_ts.len(),
                        data.len()
                    );
        ...
        while !cur_i.is_empty() {
            ...
            if cur_i.len() >= self.record_ts {
                ...
                debug!(
                    "KerberosParserTCP: more fragments required have {}, need {}",
                    cur_i.len(),
                    self.record_ts
                );
