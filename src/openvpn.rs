use nom::IResult;
use nom::HexDisplay;
use openvpn_parser::{parse_openvpn_tcp,parse_openvpn_udp,Payload};
use tls::TlsParser;

use rparser::{RParser,R_STATUS_OK,R_STATUS_FAIL};

pub struct OpenVPNParser<'a> {
    _name: Option<&'a[u8]>,
    defrag_buf: Vec<u8>,
...
impl<'a> OpenVPNParser<'a> {
    pub fn new(name: &'a[u8]) -> OpenVPNParser<'a> {
        OpenVPNParser{
            _name: Some(name),
            defrag_buf: Vec::new(),
...
impl<'a> RParser for OpenVPNParser<'a> {
...
pub fn openvpn_probe(i: &[u8]) -> bool {
